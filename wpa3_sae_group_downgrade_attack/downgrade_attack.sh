...
#Custom function. Execute WPA3 group downgrade attack
...
function python3_downgrade_attack_script_validation() {
...
function python3_downgrade_attack_validation() {
...
#Custom function. Prepare and launch WPA3 group downgrade attack
function downgrade_attack_option() {
	...
	if ! python3_downgrade_attack_validation; then
		return 1
	fi

	if ! python3_downgrade_attack_script_validation; then
		return 1
	fi
...
function downgrade_attack_prehook_hookable_for_languages() {
	...
	if [ "${arr['ENGLISH',756]}" = "6.  WPA3 attack (use a plugin here)" ]; then
		...
		arr["SPANISH",756]="6.  Ataque de downgrade de grupo WPA3"
		...
		arr["GERMAN",756]="${pending_of_translation} 6.  WPA3 group downgrade Angriff"
		...
		arr["SPANISH",757]="7.  Ataque de downgrade de grupo WPA3"
		...
		arr["GERMAN",757]="${pending_of_translation} 7.  WPA3 group downgrade Angriff"
		...
		arr["SPANISH",812]="8.  Ataque de downgrade de grupo WPA3"
		...
		arr["GERMAN",812]="${pending_of_translation} 8.  WPA3 group downgrade Angriff"
	...
	arr["SPANISH","downgrade_attack_1"]="El downgrade de grupo WPA3 fuerza al AP a aceptar grupos MODP débiles (22/23)"
	...
	arr["GERMAN","downgrade_attack_1"]="${pending_of_translation} WPA3 group downgrade zwingt den AP zur Annahme schwacher MODP-Gruppen (22/23)"
	...
	arr["GERMAN","downgrade_attack_2"]="${pending_of_translation} Für diesen Angriff muss python3.6+ auf dem System installiert sein"
	...
	arr["GERMAN","downgrade_attack_3"]="${pending_of_translation} Das python3-Skript, das für diesen Angriff erforderlich ist, fehlt. Bitte stelle sicher, dass die Datei \"${normal_color}downgrade_attack.py${red_color}\" vorhanden ist und sich im Plugin-Ordner neben der Datei \"${normal_color}downgrade_attack.sh${red_color}\" befindet"
...
#Posthook for initialize_colors function (no-op placeholder)
